...
function stringify(token2) {
  ...
  if (token2.overriddenName) {
    return `${token2.overriddenName}`;
  }
  if (token2.name) {
    return `${token2.name}`;
  }
  const res = token2.toString();
  if (res == null) {
    return "" + res;
  }
...
function throwCyclicDependencyError(token2, path) {
  const depPath = path ? `. Dependency path: ${path.join(" > ")} > ${token2}` : "";
  throw new RuntimeError(-200, `Circular dependency in DI detected for ${token2}${depPath}`);
}
...
function throwProviderNotFoundError(token2, injectorName) {
  ...
  throw new RuntimeError(-201, ngDevMode && `No provider for ${stringifyForError(token2)} found${injectorDetails}`);
}
...
function __defineInjectable(opts) {
  return {
    token: opts.token,
...
function __defineInjector(options) {
  return {
...
var NG_PROV_DEF = /* @__PURE__ */ getClosureSafeProperty({
  _prov: getClosureSafeProperty
});
var NG_INJ_DEF = /* @__PURE__ */ getClosureSafeProperty({
  _inj: getClosureSafeProperty
});
...
function injectRootLimpMode(token2, notFoundValue, flags) {
  ...
  throwProviderNotFoundError(stringify(token2), "Injector");
}
function assertInjectImplementationNotEqual(fn) {
  ngDevMode && assertNotEqual(_injectImplementation, fn, "Calling __inject would cause infinite recursion");
}
...
function emitProviderConfiguredEvent(eventProvider, isViewProvider = false) {
  ...
  if (eventProvider instanceof InjectionToken) {
    provider = eventProvider._prov || eventProvider;
  }
  injectorProfiler({
    type: 2,
...
function injectInjectorOnly(token2, flags = InjectFlags.Default) {
  if (_currentInjector === void 0) {
    ...
    const value = _currentInjector.get(token2, flags & InjectFlags.Optional ? null : void 0, flags);
    ngDevMode && emitInjectEvent(token2, value, flags);
    return value;
  }
}
function __inject(token2, flags = InjectFlags.Default) {
  return (getInjectImplementation() || injectInjectorOnly)(resolveForwardRef(token2), flags);
}
function inject(token2, flags = InjectFlags.Default) {
  return __inject(token2, convertToBitFlags(flags));
}
...
function injectArgs(types) {
  ...
  for (let i = 0; i < types.length; i++) {
    ...
    if (Array.isArray(arg)) {
      ...
      args.push(__inject(type, flags));
    } else {
      args.push(__inject(arg));
    }
  }
  return args;
}
...
function catchInjectorError(e, token2, injectorErrorName, source) {
  const tokenPath = e[NG_TEMP_TOKEN_PATH];
  if (token2[SOURCE]) {
    tokenPath.unshift(token2[SOURCE]);
  }
...
function __defineComponent(componentDefinition) {
...
function __defineDirective(directiveDefinition) {
  return noSideEffects(() => {
...
function getNgModuleDef(type, throwNotFound) {
  ...
  if (!ngModuleDef && throwNotFound === true) {
    throw new Error(`Type ${stringify(type)} does not have '_mod' property.`);
  }
  return ngModuleDef;
}
...
function assertComponentType(actual, msg = "Type passed in is not ComponentType, it does not have '_cmp' property.") {
...
function getFactoryDef(type, throwNotFound) {
  ...
  if (!hasFactoryDef && throwNotFound === true && ngDevMode) {
    throw new Error(`Type ${stringify(type)} does not have '_fac' property.`);
  }
...
__NgOnChangesFeature.ngInherit = true;
...
function lookupTokenUsingModuleInjector(lView, token2, flags, notFoundValue) {
  ...
  if ((flags & (InjectFlags.Self | InjectFlags.Host)) === 0) {
    ...
    try {
      if (moduleInjector) {
        return moduleInjector.get(token2, notFoundValue, flags & InjectFlags.Optional);
      } else {
        return injectRootLimpMode(token2, notFoundValue, flags & InjectFlags.Optional);
      }
    } finally {
...
function getOrCreateInjectable(tNode, lView, token2, flags = InjectFlags.Default, notFoundValue) {
  if (tNode !== null) {
    if (lView[FLAGS] & 2048 && // The token must be present on the current node injector when the `Self`
      // flag is set, so the lookup on embedded view injector(s) can be skipped.
      !(flags & InjectFlags.Self)) {
      const embeddedInjectorValue = lookupTokenUsingEmbeddedInjector(tNode, lView, token2, flags, NOT_FOUND);
      if (embeddedInjectorValue !== NOT_FOUND) {
        return embeddedInjectorValue;
      }
    }
    const value = lookupTokenUsingNodeInjector(tNode, lView, token2, flags, NOT_FOUND);
    if (value !== NOT_FOUND) {
      return value;
    }
  }
...
function lookupTokenUsingNodeInjector(tNode, lView, token2, flags, notFoundValue) {
  ...
  if (typeof bloomHash === "function") {
    if (!enterDI(lView, tNode, flags)) {
      return flags & InjectFlags.Host ? notFoundValueOrThrow(notFoundValue, token2, flags) : lookupTokenUsingModuleInjector(lView, token2, flags, notFoundValue);
    }
    try {
      let value;
      if (ngDevMode) {
...
function bloomHashBitOrFactory(token2) {
  ...
  const tokenId = (
    ...
    token2.hasOwnProperty(NG_ELEMENT_ID) ? token2[NG_ELEMENT_ID] : void 0
  );
...
function __getInheritedFactory(type) {
  return noSideEffects(() => {
...
function lookupTokenUsingEmbeddedInjector(tNode, lView, token2, flags, notFoundValue) {
  ...
  while (currentTNode !== null && currentLView !== null && currentLView[FLAGS] & 2048 && !(currentLView[FLAGS] & 512)) {
    ...
    const nodeInjectorValue = lookupTokenUsingNodeInjector(currentTNode, currentLView, token2, flags | InjectFlags.Self, NOT_FOUND);
    ...
    if (!parentTNode) {
      ...
      if (embeddedViewInjector) {
        const embeddedViewInjectorValue = embeddedViewInjector.get(token2, NOT_FOUND, flags);
...
var NullInjector = class {
  get(token2, notFoundValue = THROW_IF_NOT_FOUND) {
    if (notFoundValue === THROW_IF_NOT_FOUND) {
      const error = new Error(`NullInjectorError: No provider for ${stringify(token2)}!`);
...
function importProvidersFrom(...sources) {
  return {
    _providers: internalImportProvidersFrom(true, sources),
    _fromNgModule: true
  };
}
...
var R3Injector = class extends EnvironmentInjector {
  ...
  get(token2, notFoundValue = THROW_IF_NOT_FOUND, flags = InjectFlags.Default) {
    this.assertNotDestroyed();
    ...
    try {
      if (!(flags & InjectFlags.SkipSelf)) {
        ...
        if (record != null) {
          return this.hydrate(token2, record);
        }
      }
      ...
      if (e.name === "NullInjectorError") {
        ...
        path.unshift(stringify(token2));
        if (previousInjector) {
          throw e;
        } else {
          return catchInjectorError(e, token2, "R3InjectorError", this.source);
        }
      } else {
        throw e;
      }
  ...
  hydrate(token2, record) {
    if (ngDevMode && record.value === CIRCULAR) {
      throwCyclicDependencyError(stringify(token2));
...
function injectableDefOrInjectorDefFactory(token2) {
  const injectableDef = getInjectableDef(token2);
  const factory2 = injectableDef !== null ? injectableDef.factory : getFactoryDef(token2);
  if (factory2 !== null) {
    return factory2;
  }
  if (token2 instanceof InjectionToken) {
    throw new RuntimeError(204, ngDevMode && `Token ${stringify(token2)} is missing a _prov definition.`);
  }
...
function getUndecoratedInjectableFactory(token2) {
  ...
  if (paramLength > 0) {
    ...
    throw new RuntimeError(204, ngDevMode && `Can't resolve all parameters for ${stringify(token2)}: (${args.join(", ")}).`);
  }
...
function providerToFactory(provider, ngModuleType, providers) {
  ...
  if (isTypeProvider(provider)) {
    ...
    if (isValueProvider(provider)) {
      ...
      factory2 = () => __inject(resolveForwardRef(provider.useExisting));
    } else {
      const classRef = resolveForwardRef(provider && (provider.useClass || provider.provide));
...
function forEachSingleProvider(providers, fn) {
  for (const provider of providers) {
    if (Array.isArray(provider)) {
      ...
      forEachSingleProvider(provider._providers, fn);
    } else {
      fn(provider);
    }
  }
...
var Injector = /* @__PURE__ */ (() => {
  ...
  _Injector._prov = __defineInjectable({
    token: _Injector,
    providedIn: "any",
    factory: () => __inject(INJECTOR)
  });
...
function isComponentInstance(instance) {
  return instance && instance.constructor && instance.constructor._cmp;
}
function isDirectiveInstance(instance) {
  return instance && instance.constructor && instance.constructor._dir;
}
...
var Sanitizer = /* @__PURE__ */ (() => {
  ...
  _Sanitizer._prov = __defineInjectable({
    token: _Sanitizer,
    providedIn: "root",
    factory: () => null
  });
...
function __directiveInject(token2, flags = InjectFlags.Default) {
  const lView = getLView();
  if (lView === null) {
    ngDevMode && assertInjectImplementationNotEqual(__directiveInject);
    return __inject(token2, flags);
  }
...
function configureViewWithDirective(tView, tNode, lView, directiveIndex, def) {
  ...
  const nodeInjectorFactory = new NodeInjectorFactory(directiveFactory, isComponentDef(def), __directiveInject);
...
var AfterRenderEventManager = /* @__PURE__ */ (() => {
  ...
  _AfterRenderEventManager._prov = __defineInjectable({
...
var ChainedInjector = class {
  ...
  get(token2, notFoundValue, flags) {
    flags = convertToBitFlags(flags);
    const value = this.injector.get(token2, NOT_FOUND_CHECK_ONLY_ELEMENT_INJECTOR, flags);
    if (value !== NOT_FOUND_CHECK_ONLY_ELEMENT_INJECTOR || notFoundValue === NOT_FOUND_CHECK_ONLY_ELEMENT_INJECTOR) {
      return value;
    }
    return this.parentInjector.get(token2, notFoundValue, flags);
  }
};
...
function __property(propName, value, sanitizer) {
  const lView = getLView();
...
function __styleProp(prop, value, suffix) {
...
function __elementStart(index, name, attrsIndex, localRefsIndex) {
  const lView = getLView();
  const tView = getTView();
  ...
  return __elementStart;
}
function __elementEnd() {
...
function __element(index, name, attrsIndex, localRefsIndex) {
  __elementStart(index, name, attrsIndex, localRefsIndex);
  __elementEnd();
  return __element;
}
...
function __listener(eventName, listenerFn, useCapture, eventTargetResolver) {
  const lView = getLView();
  const tView = getTView();
  const tNode = getCurrentTNode();
...
function __textInterpolate1(prefix, v0, suffix) {
  const lView = getLView();
  const interpolated = interpolation1(lView, prefix, v0, suffix);
  if (interpolated !== NO_CHANGE) {
    textBindingInternal(lView, getSelectedIndex(), interpolated);
  }
  return __textInterpolate1;
}
...
function resolveProvider(provider, tInjectables, lInjectablesBlueprint, isComponent, isViewProvider) {
  ...
  if (Array.isArray(provider)) {
    ...
    if (isTypeProvider(provider) || !provider.multi) {
      const factory2 = new NodeInjectorFactory(providerFactory, isViewProvider, __directiveInject);
      ...
      if (isViewProvider && !doesViewProvidersFactoryExist || !isViewProvider && !doesProvidersFactoryExist) {
        diPublicInInjector(getOrCreateNodeInjectorForNode(tNode, lView), tView, token2);
        const factory2 = multiFactory(isViewProvider ? multiViewProvidersFactoryResolver : multiProvidersFactoryResolver, lInjectablesBlueprint.length, isViewProvider, isComponent, providerFactory);
...
function multiFactory(factoryFn, index, isViewProvider, isComponent, f) {
  const factory2 = new NodeInjectorFactory(factoryFn, isViewProvider, __directiveInject);
  factory2.multi = [];
  factory2.index = index;
  factory2.componentProviders = 0;
  multiFactoryAdd(factory2, f, isComponent && !isViewProvider);
  return factory2;
}
function __ProvidersFeature(providers, viewProviders = []) {
  return (definition) => {
...
var StandaloneService = /* @__PURE__ */ (() => {
  ...
  _StandaloneService._prov = __defineInjectable({
    token: _StandaloneService,
    providedIn: "environment",
    factory: () => new _StandaloneService(__inject(EnvironmentInjector))
  });
  let StandaloneService2 = _StandaloneService;
  return StandaloneService2;
})();
function __StandaloneFeature(definition) {
...
function __pureFunction0(slotOffset, pureFn, thisArg) {
...
function __pipe(index, pipeName) {
  const tView = getTView();
  let pipeDef;
  const adjustedIndex = index + HEADER_OFFSET;
  if (tView.firstCreatePass) {
  ...
  const previousInjectImplementation = setInjectImplementation(__directiveInject);
  try {
...
function __pipeBind1(index, offset, v1) {
  const adjustedIndex = index + HEADER_OFFSET;
  const lView = getLView();
...
var ApplicationInitStatus = /* @__PURE__ */ (() => {
  ...
  _ApplicationInitStatus._fac = function ApplicationInitStatus_Factory(t) {
    return new (t || _ApplicationInitStatus)();
  };
  _ApplicationInitStatus._prov = /* @__PURE__ */ __defineInjectable({
    token: _ApplicationInitStatus,
    factory: _ApplicationInitStatus._fac,
    providedIn: "root"
  });
...
var Console = /* @__PURE__ */ (() => {
  ...
  _Console._fac = function Console_Factory(t) {
    return new (t || _Console)();
  };
  _Console._prov = /* @__PURE__ */ __defineInjectable({
    token: _Console,
    factory: _Console._fac,
    providedIn: "platform"
  });
...
var ImagePerformanceWarning = /* @__PURE__ */ (() => {
  ...
  _ImagePerformanceWarning._fac = function ImagePerformanceWarning_Factory(t) {
    return new (t || _ImagePerformanceWarning)();
  };
  _ImagePerformanceWarning._prov = /* @__PURE__ */ __defineInjectable({
    token: _ImagePerformanceWarning,
    factory: _ImagePerformanceWarning._fac,
    providedIn: "root"
  });
...
var InitialRenderPendingTasks = /* @__PURE__ */ (() => {
  ...
  _InitialRenderPendingTasks._fac = function InitialRenderPendingTasks_Factory(t) {
    return new (t || _InitialRenderPendingTasks)();
  };
  _InitialRenderPendingTasks._prov = /* @__PURE__ */ __defineInjectable({
    token: _InitialRenderPendingTasks,
    factory: _InitialRenderPendingTasks._fac,
    providedIn: "root"
  });
...
var Compiler = /* @__PURE__ */ (() => {
  ...
  _Compiler._fac = function Compiler_Factory(t) {
    return new (t || _Compiler)();
  };
  _Compiler._prov = /* @__PURE__ */ __defineInjectable({
    token: _Compiler,
    factory: _Compiler._fac,
    providedIn: "root"
  });
...
function publishDefaultGlobalUtils$1() {
  if (!_published) {
    ...
    publishGlobalUtil("_getDependenciesFromInjectable", getDependenciesFromInjectable);
    publishGlobalUtil("_getInjectorProviders", getInjectorProviders);
    publishGlobalUtil("_getInjectorResolutionPath", getInjectorResolutionPath);
    publishGlobalUtil("_getInjectorMetadata", getInjectorMetadata);
    publishGlobalUtil("_setProfiler", setProfiler);
...
var ApplicationRef = /* @__PURE__ */ (() => {
  ...
  _ApplicationRef._fac = function ApplicationRef_Factory(t) {
    return new (t || _ApplicationRef)();
  };
  _ApplicationRef._prov = /* @__PURE__ */ __defineInjectable({
    token: _ApplicationRef,
    factory: _ApplicationRef._fac,
    providedIn: "root"
  });
...
var NgZoneChangeDetectionScheduler = /* @__PURE__ */ (() => {
  ...
  _NgZoneChangeDetectionScheduler._fac = function NgZoneChangeDetectionScheduler_Factory(t) {
    return new (t || _NgZoneChangeDetectionScheduler)();
  };
  _NgZoneChangeDetectionScheduler._prov = /* @__PURE__ */ __defineInjectable({
    token: _NgZoneChangeDetectionScheduler,
    factory: _NgZoneChangeDetectionScheduler._fac,
    providedIn: "root"
  });
...
var PlatformLocation = /* @__PURE__ */ (() => {
  ...
  _PlatformLocation._fac = function PlatformLocation_Factory(t) {
    return new (t || _PlatformLocation)();
  };
  _PlatformLocation._prov = /* @__PURE__ */ __defineInjectable({
...
var BrowserPlatformLocation = /* @__PURE__ */ (() => {
  ...
  _BrowserPlatformLocation._fac = function BrowserPlatformLocation_Factory(t) {
    return new (t || _BrowserPlatformLocation)();
  };
  _BrowserPlatformLocation._prov = /* @__PURE__ */ __defineInjectable({
...
var LocationStrategy = /* @__PURE__ */ (() => {
  ...
  _LocationStrategy._fac = function LocationStrategy_Factory(t) {
    return new (t || _LocationStrategy)();
  };
  _LocationStrategy._prov = /* @__PURE__ */ __defineInjectable({
...
var PathLocationStrategy = /* @__PURE__ */ (() => {
  ...
  _PathLocationStrategy._fac = function PathLocationStrategy_Factory(t) {
    return new (t || _PathLocationStrategy)(__inject(PlatformLocation), __inject(APP_BASE_HREF, 8));
  };
  _PathLocationStrategy._prov = /* @__PURE__ */ __defineInjectable({
    token: _PathLocationStrategy,
    factory: _PathLocationStrategy._fac,
    providedIn: "root"
  });
...
var Location = /* @__PURE__ */ (() => {
  ...
  _Location._fac = function Location_Factory(t) {
    return new (t || _Location)(__inject(LocationStrategy));
  };
  _Location._prov = /* @__PURE__ */ __defineInjectable({
    token: _Location,
...
var JsonPipe = /* @__PURE__ */ (() => {
  ...
  _JsonPipe._fac = function JsonPipe_Factory(t) {
    return new (t || _JsonPipe)();
  };
  _JsonPipe._pipe = /* @__PURE__ */ __definePipe({
    name: "json",
    type: _JsonPipe,
    pure: false,
    standalone: true
...
var CommonModule = /* @__PURE__ */ (() => {
  ...
  _CommonModule._fac = function CommonModule_Factory(t) {
    return new (t || _CommonModule)();
  };
  _CommonModule._mod = /* @__PURE__ */ __defineNgModule({
    type: _CommonModule
  });
  _CommonModule._inj = /* @__PURE__ */ __defineInjector({});
...
var BrowserXhr = /* @__PURE__ */ (() => {
  ...
  _BrowserXhr._fac = function BrowserXhr_Factory(t) {
    return new (t || _BrowserXhr)();
  };
  _BrowserXhr._prov = /* @__PURE__ */ __defineInjectable({
    token: _BrowserXhr,
    factory: _BrowserXhr._fac
  });
...
var EventManager = /* @__PURE__ */ (() => {
  ...
  _EventManager._fac = function EventManager_Factory(t) {
    return new (t || _EventManager)(__inject(EVENT_MANAGER_PLUGINS), __inject(NgZone));
  };
  _EventManager._prov = /* @__PURE__ */ __defineInjectable({
    token: _EventManager,
    factory: _EventManager._fac
  });
...
var SharedStylesHost = /* @__PURE__ */ (() => {
  ...
  _SharedStylesHost._fac = function SharedStylesHost_Factory(t) {
    return new (t || _SharedStylesHost)(__inject(DOCUMENT2), __inject(APP_ID), __inject(CSP_NONCE, 8), __inject(PLATFORM_ID));
  };
  _SharedStylesHost._prov = /* @__PURE__ */ __defineInjectable({
    token: _SharedStylesHost,
    factory: _SharedStylesHost._fac
  });
...
var DomRendererFactory2 = /* @__PURE__ */ (() => {
  ...
  _DomRendererFactory2._fac = function DomRendererFactory2_Factory(t) {
    return new (t || _DomRendererFactory2)(__inject(EventManager), __inject(SharedStylesHost), __inject(APP_ID), __inject(REMOVE_STYLES_ON_COMPONENT_DESTROY), __inject(DOCUMENT2), __inject(PLATFORM_ID), __inject(NgZone), __inject(CSP_NONCE));
  };
  _DomRendererFactory2._prov = /* @__PURE__ */ __defineInjectable({
    token: _DomRendererFactory2,
    factory: _DomRendererFactory2._fac
  });
...
var DomEventsPlugin = /* @__PURE__ */ (() => {
  ...
  _DomEventsPlugin._fac = function DomEventsPlugin_Factory(t) {
    return new (t || _DomEventsPlugin)(__inject(DOCUMENT2));
  };
  _DomEventsPlugin._prov = /* @__PURE__ */ __defineInjectable({
    token: _DomEventsPlugin,
    factory: _DomEventsPlugin._fac
  });
...
var KeyEventsPlugin = /* @__PURE__ */ (() => {
  ...
  _KeyEventsPlugin._fac = function KeyEventsPlugin_Factory(t) {
    return new (t || _KeyEventsPlugin)(__inject(DOCUMENT2));
  };
  _KeyEventsPlugin._prov = /* @__PURE__ */ __defineInjectable({
    token: _KeyEventsPlugin,
    factory: _KeyEventsPlugin._fac
  });
...
function createTitle() {
  return new Title(__inject(DOCUMENT2));
}
var Title = /* @__PURE__ */ (() => {
  ...
  _Title._fac = function Title_Factory(t) {
    return new (t || _Title)(__inject(DOCUMENT2));
  };
  _Title._prov = /* @__PURE__ */ __defineInjectable({
    token: _Title,
...
var UrlSerializer = /* @__PURE__ */ (() => {
  ...
  _UrlSerializer._fac = function UrlSerializer_Factory(t) {
    return new (t || _UrlSerializer)();
  };
  _UrlSerializer._prov = /* @__PURE__ */ __defineInjectable({
...
var ChildrenOutletContexts = /* @__PURE__ */ (() => {
  ...
  _ChildrenOutletContexts._fac = function ChildrenOutletContexts_Factory(t) {
    return new (t || _ChildrenOutletContexts)();
  };
  _ChildrenOutletContexts._prov = /* @__PURE__ */ __defineInjectable({
    token: _ChildrenOutletContexts,
    factory: _ChildrenOutletContexts._fac,
    providedIn: "root"
  });
...
var RouterOutlet = /* @__PURE__ */ (() => {
  ...
  _RouterOutlet._fac = function RouterOutlet_Factory(t) {
    return new (t || _RouterOutlet)();
  };
  _RouterOutlet._dir = /* @__PURE__ */ __defineDirective({
    type: _RouterOutlet,
...
var OutletInjector = class {
  ...
  get(token2, notFoundValue) {
    if (token2 === ActivatedRoute) {
      return this.route;
    }
    if (token2 === ChildrenOutletContexts) {
      return this.childContexts;
    }
    return this.parent.get(token2, notFoundValue);
  }
};
...
var _EmptyOutletComponent = /* @__PURE__ */ (() => {
  const __EmptyOutletComponent = class __EmptyOutletComponent {
  };
  __EmptyOutletComponent._fac = function _EmptyOutletComponent_Factory(t) {
    return new (t || __EmptyOutletComponent)();
  };
  __EmptyOutletComponent._cmp = /* @__PURE__ */ __defineComponent({
    type: __EmptyOutletComponent,
  ...
  let _EmptyOutletComponent2 = __EmptyOutletComponent;
  return _EmptyOutletComponent2;
})();
...
var TitleStrategy = /* @__PURE__ */ (() => {
  ...
  _TitleStrategy._fac = function TitleStrategy_Factory(t) {
    return new (t || _TitleStrategy)();
  };
  _TitleStrategy._prov = /* @__PURE__ */ __defineInjectable({
...
var DefaultTitleStrategy = /* @__PURE__ */ (() => {
  ...
  _DefaultTitleStrategy._fac = function DefaultTitleStrategy_Factory(t) {
    return new (t || _DefaultTitleStrategy)(__inject(Title));
  };
  _DefaultTitleStrategy._prov = /* @__PURE__ */ __defineInjectable({
    token: _DefaultTitleStrategy,
    factory: _DefaultTitleStrategy._fac,
    providedIn: "root"
  });
...
var RouterConfigLoader = /* @__PURE__ */ (() => {
  ...
  _RouterConfigLoader._fac = function RouterConfigLoader_Factory(t) {
    return new (t || _RouterConfigLoader)();
  };
  _RouterConfigLoader._prov = /* @__PURE__ */ __defineInjectable({
    token: _RouterConfigLoader,
    factory: _RouterConfigLoader._fac,
    providedIn: "root"
  });
...
var UrlHandlingStrategy = /* @__PURE__ */ (() => {
  ...
  _UrlHandlingStrategy._fac = function UrlHandlingStrategy_Factory(t) {
    return new (t || _UrlHandlingStrategy)();
  };
  _UrlHandlingStrategy._prov = /* @__PURE__ */ __defineInjectable({
...
var DefaultUrlHandlingStrategy = /* @__PURE__ */ (() => {
  ...
  _DefaultUrlHandlingStrategy._fac = function DefaultUrlHandlingStrategy_Factory(t) {
    return new (t || _DefaultUrlHandlingStrategy)();
  };
  _DefaultUrlHandlingStrategy._prov = /* @__PURE__ */ __defineInjectable({
    token: _DefaultUrlHandlingStrategy,
    factory: _DefaultUrlHandlingStrategy._fac,
    providedIn: "root"
  });
...
var NavigationTransitions = /* @__PURE__ */ (() => {
  ...
  _NavigationTransitions._fac = function NavigationTransitions_Factory(t) {
    return new (t || _NavigationTransitions)();
  };
  _NavigationTransitions._prov = /* @__PURE__ */ __defineInjectable({
    token: _NavigationTransitions,
    factory: _NavigationTransitions._fac,
    providedIn: "root"
  });
...
var RouteReuseStrategy = /* @__PURE__ */ (() => {
  ...
  _RouteReuseStrategy._fac = function RouteReuseStrategy_Factory(t) {
    return new (t || _RouteReuseStrategy)();
  };
  _RouteReuseStrategy._prov = /* @__PURE__ */ __defineInjectable({
...
var DefaultRouteReuseStrategy = /* @__PURE__ */ (() => {
  ...
  _DefaultRouteReuseStrategy._fac = /* @__PURE__ */ (() => {
    let _DefaultRouteReuseStrategy_BaseFactory;
    return function DefaultRouteReuseStrategy_Factory(t) {
      return (_DefaultRouteReuseStrategy_BaseFactory || (_DefaultRouteReuseStrategy_BaseFactory = __getInheritedFactory(_DefaultRouteReuseStrategy)))(t || _DefaultRouteReuseStrategy);
    };
  })();
  _DefaultRouteReuseStrategy._prov = /* @__PURE__ */ __defineInjectable({
    token: _DefaultRouteReuseStrategy,
    factory: _DefaultRouteReuseStrategy._fac,
    providedIn: "root"
  });
...
var StateManager = /* @__PURE__ */ (() => {
  ...
  _StateManager._fac = function StateManager_Factory(t) {
    return new (t || _StateManager)();
  };
  _StateManager._prov = /* @__PURE__ */ __defineInjectable({
    token: _StateManager,
...
var HistoryStateManager = /* @__PURE__ */ (() => {
  const _HistoryStateManager = class _HistoryStateManager extends StateManager {
    ...
    get browserPageId() {
      ...
      return this.restoredState()?._routerPageId ?? this.currentPageId;
    }
    getRouterState() {
  ...
  _HistoryStateManager._fac = /* @__PURE__ */ (() => {
    let _HistoryStateManager_BaseFactory;
    return function HistoryStateManager_Factory(t) {
      return (_HistoryStateManager_BaseFactory || (_HistoryStateManager_BaseFactory = __getInheritedFactory(_HistoryStateManager)))(t || _HistoryStateManager);
    };
  })();
  _HistoryStateManager._prov = /* @__PURE__ */ __defineInjectable({
    token: _HistoryStateManager,
    factory: _HistoryStateManager._fac,
    providedIn: "root"
  });
...
var Router = /* @__PURE__ */ (() => {
  const _Router = class _Router {
    ...
    navigateToSyncWithBrowser(url, source, state) {
      ...
      if (state) {
        ...
        delete stateCopy._routerPageId;
  ...
  _Router._fac = function Router_Factory(t) {
    return new (t || _Router)();
  };
  _Router._prov = /* @__PURE__ */ __defineInjectable({
    token: _Router,
    factory: _Router._fac,
    providedIn: "root"
  });
  let Router2 = _Router;
  return Router2;
...
var token = new InjectionToken("some token");
var factory = new InjectionToken("some factory");

// src/app/module0/services/zero-component.service.ts
var ZeroComponentService = class {
  constructor() {
    console.log("ZeroComponentService init");
  }
};

// src/app/module0/services/zero-root.service.ts
var ZeroRootService = /* @__PURE__ */ (() => {
  const _ZeroRootService = class _ZeroRootService {
    constructor() {
      console.log("ZeroRootService init");
    }
  };
  _ZeroRootService._fac = function ZeroRootService_Factory(t) {
    return new (t || _ZeroRootService)();
  };
  _ZeroRootService._prov = /* @__PURE__ */ __defineInjectable({
    token: _ZeroRootService,
    factory: _ZeroRootService._fac,
    providedIn: "root"
  });
  let ZeroRootService2 = _ZeroRootService;
  return ZeroRootService2;
})();

// src/app/module0/components/zero-component-a/zero-component-a.component.ts
var ZeroComponentAComponent = /* @__PURE__ */ (() => {
  const _ZeroComponentAComponent = class _ZeroComponentAComponent {
    constructor(_zeroAComponentService, _zeroARootService) {
    }
  };
  _ZeroComponentAComponent._fac = function ZeroComponentAComponent_Factory(t) {
    return new (t || _ZeroComponentAComponent)(__directiveInject(ZeroComponentService), __directiveInject(ZeroRootService));
  };
  _ZeroComponentAComponent._cmp = /* @__PURE__ */ __defineComponent({
    type: _ZeroComponentAComponent,
    selectors: [["zero-component-a"]],
    decls: 2,
    vars: 0,
    template: function ZeroComponentAComponent_Template(rf, ctx) {
      if (rf & 1) {
        __elementStart(0, "div");
        __text(1, " zero-component-a.component\n");
        __elementEnd();
      }
    },
    encapsulation: 2,
  ...
  let ZeroComponentAComponent2 = _ZeroComponentAComponent;
  return ZeroComponentAComponent2;
})();

// src/app/module0/services/zero-with-injector.service.ts
var ZeroWithInjectorService = /* @__PURE__ */ (() => {
  const _ZeroWithInjectorService = class _ZeroWithInjectorService {
    constructor() {
      console.log("ZeroWithInjectorService init");
    }
  };
  _ZeroWithInjectorService._fac = function ZeroWithInjectorService_Factory(t) {
    return new (t || _ZeroWithInjectorService)();
  };
  _ZeroWithInjectorService._prov = /* @__PURE__ */ __defineInjectable({
    token: _ZeroWithInjectorService,
    factory: _ZeroWithInjectorService._fac
  });
  let ZeroWithInjectorService2 = _ZeroWithInjectorService;
  return ZeroWithInjectorService2;
})();

// src/app/module0/components/zero-component-b/zero-component-b.component.ts
var _c0 = () => ({
  "key": "value-to-json"
});
var ZeroComponentBComponent = /* @__PURE__ */ (() => {
  const _ZeroComponentBComponent = class _ZeroComponentBComponent {
    constructor(_zeroBWithInjectorService) {
    }
  };
  _ZeroComponentBComponent._fac = function ZeroComponentBComponent_Factory(t) {
    return new (t || _ZeroComponentBComponent)(__directiveInject(ZeroWithInjectorService));
  };
  _ZeroComponentBComponent._cmp = /* @__PURE__ */ __defineComponent({
    type: _ZeroComponentBComponent,
    selectors: [["zero-component-b"]],
    decls: 3,
    vars: 4,
    template: function ZeroComponentBComponent_Template(rf, ctx) {
      if (rf & 1) {
        __elementStart(0, "div");
        __text(1);
        __pipe(2, "json");
      ...
      if (rf & 2) {
        __advance(1);
        __textInterpolate1(" zero-component-b.component ", __pipeBind1(2, 1, __pureFunction0(3, _c0)), "\n");
      }
    },
    dependencies: [JsonPipe],
    encapsulation: 2
  });
  let ZeroComponentBComponent2 = _ZeroComponentBComponent;
  return ZeroComponentBComponent2;
})();

// src/app/module0/components/zero-component-c/zero-component-c.component.ts
var ZeroComponentCComponent = /* @__PURE__ */ (() => {
  const _ZeroComponentCComponent = class _ZeroComponentCComponent {
    redColor() {
  ...
  _ZeroComponentCComponent._fac = function ZeroComponentCComponent_Factory(t) {
    return new (t || _ZeroComponentCComponent)();
  };
  _ZeroComponentCComponent._cmp = /* @__PURE__ */ __defineComponent({
    type: _ZeroComponentCComponent,
    selectors: [["zero-component-c"]],
    hostVars: 2,
    hostBindings: function ZeroComponentCComponent_HostBindings(rf, ctx) {
      if (rf & 1) {
        __listener("mouseenter", function ZeroComponentCComponent_mouseenter_HostBindingHandler() {
          return ctx.mouseEnter();
        });
      }
      if (rf & 2) {
        __styleProp("color", ctx.redColor);
      }
    },
    inputs: {
      some: "some"
    },
    features: [__ProvidersFeature([{
      provide: "ZERO_COMPONENT_C_PROVIDE",
      useValue: "ZERO_COMPONENT_C_VALUE"
    }]), __NgOnChangesFeature],
    decls: 4,
    vars: 0,
    consts: [[3, "click"]],
    template: function (rf, ctx) {
      function ZeroComponentCComponent_Template(rf, ctx) {
        if (rf & 1) {
          __elementStart(0, "div");
          __text(1, " zero-component-c.component ");
          __elementStart(2, "button", 0);
          __listener("click", function ZeroComponentCComponent_Template_button_click_2_listener() {
      ...
      const result = ZeroComponentCComponent_Template(rf, ctx);
      console.dir(ctx)
      console.dir(result)
      return result
    },
  ...
  console.dir(_ZeroComponentCComponent._cmp.template);
  console.log(_ZeroComponentCComponent._cmp.template);


  let ZeroComponentCComponent2 = _ZeroComponentCComponent;
  return ZeroComponentCComponent2;
})();

// src/app/module0/components/zero-component-base/zero-component-base.component.ts
var ZeroComponentBaseComponent = /* @__PURE__ */ (() => {
  const _ZeroComponentBaseComponent = class _ZeroComponentBaseComponent {
    constructor() {
    }
  };
  _ZeroComponentBaseComponent._fac = function ZeroComponentBaseComponent_Factory(t) {
    return new (t || _ZeroComponentBaseComponent)();
  };
  _ZeroComponentBaseComponent._cmp = /* @__PURE__ */ __defineComponent({
    type: _ZeroComponentBaseComponent,
    selectors: [["zero-component-base"]],
    decls: 4,
    vars: 1,
    consts: [[3, "some"]],
    template: function ZeroComponentBaseComponent_Template(rf, ctx) {
      if (rf & 1) {
        __elementStart(0, "div");
        __element(1, "zero-component-a")(2, "zero-component-b")(3, "zero-component-c", 0);
        __elementEnd();
      }
      if (rf & 2) {
        __advance(3);
    ...
    dependencies: [ZeroComponentAComponent, ZeroComponentBComponent, ZeroComponentCComponent],
    encapsulation: 2
  });
  console.dir(_ZeroComponentBaseComponent._cmp);
  let ZeroComponentBaseComponent2 = _ZeroComponentBaseComponent;
  return ZeroComponentBaseComponent2;
})();

// src/app/module0/module-zero.module.ts
var ModuleZero = /* @__PURE__ */ (() => {
  const _ModuleZero = class _ModuleZero {
  };
  _ModuleZero._fac = function ModuleZero_Factory(t) {
    return new (t || _ModuleZero)();
  };
  _ModuleZero._mod = /* @__PURE__ */ __defineNgModule({
    type: _ModuleZero
  });
  _ModuleZero._inj = /* @__PURE__ */ __defineInjector({
    providers: [{
      provide: "MODULE_ZERO_PROVIDE",
      useValue: "MODULE_ZERO_PROVIDE_VALUE"
    }, ZeroComponentService, ZeroWithInjectorService],
    imports: [CommonModule]
  });
  let ModuleZero2 = _ModuleZero;
  return ModuleZero2;
})();
...
var AppComponent = /* @__PURE__ */ (() => {
  const _AppComponent = class _AppComponent {
    constructor() {
      this.title = "iyv-angular";
    }
  };
  _AppComponent._fac = function AppComponent_Factory(t) {
    return new (t || _AppComponent)();
  };
  _AppComponent._cmp = /* @__PURE__ */ __defineComponent({
    type: _AppComponent,
    selectors: [["app-root"]],
    standalone: true,
    features: [__StandaloneFeature],
    ...
    template: function AppComponent_Template(rf, ctx) {
      if (rf & 1) {
        ...
        __element(1, "zero-component-base");
        __elementEnd();
        __element(2, "router-outlet");
      }
    },
    dependencies: [ModuleZero, ZeroComponentBaseComponent, CommonModule, RouterOutlet],
